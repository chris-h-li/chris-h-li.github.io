...
This test yields a p-value of **`r p_value_interact`**, so we reject the null and conclude that there are significant interaction effects between contrast level for left and right stimuli. In other words, the left and right stimuli do not have additive effects on neural responses. 
...
First, I test whether the session random effect should be retained in the model, with the following hypothesis. I use a likelihood ratio test with a significance level of 0.05.
...
Next I run some model diagnostics for this mixed effects model to determine whether the model assumptions are fulfilled. First I assess whether the residuals and session random effect are normally distributed around zero using QQ plots.
